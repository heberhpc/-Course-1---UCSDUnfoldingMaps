 ...
 * Date: Jun 13, 2018
 * */
public class EarthquakeCityMapV5_mod6 extends PApplet {
	...
	private void populateNuclearReactor()  {
		...
		//PRINT ALL THE NUCLEAR REACTORS
		//FOR DEBBUG = SET TRUE
	...
	private void hidedMarks() {
		...
		if (lastClicked instanceof EarthquakeMarker ) {
			...
			// define what Nuclear reactor must be displayed
		...
		//IF IS A CITY_MARKER
			...
			//Define what earthquakes must be displayed
			for (Marker e: quakeMarkers) {
		...
		// IF IS A NUCLEAR REACTOR	
			...
			//decide what nuclear markers must be displayed
			for (Marker n: nuclearReactorMarkers) {
				if (n != lastClicked) {
					n.setHidden(true);
				}
			...
			// display only dangerous earthquake
			for (Marker e: quakeMarkers ) {
				double threadDistance = ((EarthquakeMarker) e).threatCircle();
				double distanceFromlastClicked = e.getDistanceTo(lastClicked.getLocation());
