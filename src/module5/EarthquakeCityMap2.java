...
public class EarthquakeCityMap2 extends PApplet {
	...
	public void setup() {		
		...
		if (offline) {
			...
			map = new UnfoldingMap(this, 200, 50, 650, 600, selectProvider(1));
	...
	private void hiddenMarks() {
		
		System.out.println("HIDDENMARKS");
		
		System.out.println(lastClicked);
		
		if (lastClicked instanceof EarthquakeMarker ) {
			System.out.println("EARTHQUAKEMARK");
			
			//walk through every quake marker and see if its selected
			//if is not selected, set it to hidden
			...
			//walk through every city marker and set it to hidden
			// if this city is in the radius of threat 
			
			for (Marker m: cityMarkers) {
				
				double distance = lastClicked.getDistanceTo(m.getLocation());
				
				double radiusThreat = ((EarthquakeMarker) lastClicked).threatCircle();
				
				if (distance < radiusThreat) {
					System.out.println("RADIUS" + radiusThreat);
					System.out.println("DISTAN" + distance);
					System.out.println(distance < radiusThreat);
					m.setHidden(false);
				}
		...
		if (lastClicked instanceof CityMarker ) {
			System.out.println("CITYMARKER");
			
			//walk through every city marker and see if its selected
			//if is not selected, set it to hidden
			for (Marker m : cityMarkers) {
				if (m.isSelected()==false) {
					m.setHidden(true);
			...
			//walk through every quake marker and see if its a threat 
			//if is not, set it to hidden
			for (Marker m : quakeMarkers) {
				if (true) {
					m.setHidden(true);
				}
